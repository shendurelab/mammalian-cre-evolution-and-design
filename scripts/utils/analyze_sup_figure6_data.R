...
### Epas1 fasta file ###
CRE_oi <- "Epas1_chr17_10063"
fasta_file <- '../data/oCRE_fasta/Epas1_chr17_10063_oCRE.fasta'
...
#### merge phyloP scores
mouse_func_tfbs.phyloP <- epas1_mouse_msa_list$msa_func_TFBS %>%
  # join by CRE first to restrict mapping
  inner_join(singleBase.phyloP, by = "CRE") %>%
  # keep only rows where pos lies inside the TFBS range
  filter(pos >= TFBS_start & pos <= TFBS_end) %>%
  # select the relevant columns
  dplyr::select(CRE, TF_name, TFBS_start, TFBS_end, pos, phyloP, rank_TF, TFBS_orientation)

epas1_func_tfbs.phyloP <- mouse_func_tfbs.phyloP %>% filter(CRE == CRE_oi)
epas1_triplet_func_tfbs <- epas1_mouse_func_tfbs %>% filter(TFBS_start >= 214 & TFBS_end <= 254)
epas1_func_tfbs.phyloP <- epas1_func_tfbs.phyloP %>%
  rowwise() %>%
  mutate(in_triplet = any(
    epas1_triplet_func_tfbs$TFBS_start == TFBS_start &
    epas1_triplet_func_tfbs$TFBS_end == TFBS_end
  )) %>%
...
epas1_triplet_func_tfbs.phyloP_avg <- epas1_func_tfbs.phyloP %>% group_by(in_triplet) %>% 
                        summarise(avg_phyloP = mean(phyloP))
