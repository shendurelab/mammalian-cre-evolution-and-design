...
make_msa_tfbs_traj_df <- function(fasta.file, species.list, ref_name, CRE_oi, tfbs_df, func_tfbs_df){
    ...
    msa_df_TFBS <- make_cre_tfbs_alignment_dataframe(msa_df, tfbs_df, CRE_oi) %>%
                      drop_na(msa_start, msa_end) ### drop deletion events at ends
    msa_df_TFBS$species <- factor(msa_df_TFBS$species, levels = species.list[species.list%in%unique(msa_df_TFBS$species)])
...
#         overlaps <- findOverlaps(msa_ref_df_TFBS_gr, species_tfbs_gr, type = 'start')
        # Find overlaps with reference
          overlaps <- findOverlaps(msa_ref_df_TFBS_gr, species_tfbs_gr)
          query_hits <- queryHits(overlaps)
          subject_hits <- subjectHits(overlaps)

          # Construct dataframe of overlap pairs
          overlap_df <- data.frame(
            query_idx = query_hits,
            subject_idx = subject_hits,
            norm_affinity_ref = msa_ref_df_TFBS$norm_affinity[query_hits],
            rank_TF = msa_ref_df_TFBS$rank_TF[query_hits],
            ref_msa_start = msa_ref_df_TFBS$msa_start[query_hits],
            ref_msa_end = msa_ref_df_TFBS$msa_end[query_hits]
          )
        
          # Attach original species TFBS data
          overlap_df <- overlap_df %>%
            bind_cols(species_tfbs_df[subject_hits, ])

          # Select best overlap per query based on similar msa position, rank_TF, and affinity
          best_overlap_df <- overlap_df %>%
              mutate(
                msa_dist = abs(msa_start - ref_msa_start) + abs(msa_end - ref_msa_end),  # similarity to reference
                affinity_dist     = abs(norm_affinity_ref - norm_affinity),
                rank_TF = if_else(is.na(rank_TF), Inf, rank_TF)  # so NA ranks are deprioritized
              ) %>%
              group_by(query_idx) %>%
              arrange(msa_dist, rank_TF) %>%  # first by closest msa position, then by rank
              dplyr::slice(1) %>%
              ungroup() %>%
              dplyr::select(-query_idx, -msa_dist, -ref_msa_start, -ref_msa_end, -affinity_dist) %>% 
              mutate(rank_TF = if_else(is.infinite(rank_TF), NA, rank_TF))
          
          ### add back missing non-overlapping tfbs
          species_tfbs_df <- species_tfbs_df %>% mutate(subject_idx = row_number())
          missing_idx <- setdiff(species_tfbs_df$subject_idx,
                       unique(best_overlap_df$subject_idx))
          pad <- species_tfbs_df %>%
              filter(subject_idx %in% missing_idx) %>%
              mutate(
                norm_affinity_ref = 0,
                rank_TF           = NA_real_,
              )

            ## 4) Keep the same columns/order as best_overlap_df, then bind and sort
            common_cols <- union(names(best_overlap_df), names(pad))
            pad2 <- pad %>% dplyr::select(all_of(common_cols))
            best2 <- best_overlap_df %>% dplyr::select(all_of(common_cols))

            complete_overlap_df <- bind_rows(best2, pad2) %>%
              arrange(subject_idx) %>% dplyr::select(-subject_idx)
...
#         species_tfbs_df <- left_join(species_tfbs_df, species_tfbs_df_w_overlaps)
        ...
        final_df_TFBS <- bind_rows(final_df_TFBS, complete_overlap_df)
...
plot_tfbs_trajectory <- function(msa_tfbs_df, func_tfbs_df, CRE_oi, ref_name, labels, 
                                 aff_cutoff = 0.05, tfbs_size = 0.5 ,rm_species = NULL){
    ...
    final_msa_filter_tfbs_df <- final_msa_filter_tfbs_df %>% filter(!center_pos%in%unique(final_cre_func_tfbs$center_pos))
    
    library(ggtext)
    
    # draw order: low affinity first, high affinity last
    final_msa_filter_tfbs_df <- final_msa_filter_tfbs_df %>%
      arrange(species, center_pos, adjusted_affinity) %>% 
        mutate()

    final_cre_func_tfbs <- final_cre_func_tfbs %>%
      arrange(species, center_pos, adjusted_affinity)
...
#                  scale_color_manual(values = col_TFs) + 
                theme_classic() + scale_y_discrete(labels = labels) + 
        ...
        if(is.null(labels)){
            ...
            if(pic_labels){
                mpra_bar <- mpra_bar + theme(legend.position = 'bottom',legend.box="horizontal", legend.margin=margin(),
                                  axis.text.x = element_markdown(color = "black", size = 6),
                                  axis.line.x = element_blank(),
                                  axis.ticks.x = element_blank(),
                                  axis.title.x=element_blank())
            }else{
...
plot_seqID_trajectory <- function(mpra_act_df, ref_mpra_df, control_mpra_df, CRE_oi, species.list, labels = NULL, pic_labels = F, flip = F){
    species_act <- mpra_act_df %>% filter(full_CRE_id == CRE_oi) %>% filter(species%in%species.list)
    species_act$species <- factor(species_act$species, levels = species.list[species.list%in%species_act$species])
    cre_oi_wt_act <- ref_mpra_df %>% filter(full_CRE_id == CRE_oi) %>% pull(mean_MPRA_act)
    cre_oi_wt_sd <- ref_mpra_df %>% filter(full_CRE_id == CRE_oi) %>% pull(sd_MPRA_act)
    ...
    minP_oi_sd <- control_mpra_df %>% filter(CRE_id == 'minP') %>% pull(sd_MPRA_act)
    if(flip){
        seq_bar <- ggplot(species_act, aes(y=seqid, x=species)) + #, color=mean_MPRA_act)) + 
            geom_point(size = 2) + 
        #         geom_rect(xmin = cre_oi_wt_act-cre_oi_wt_sd, xmax = cre_oi_wt_act+cre_oi_wt_sd, ymin = -Inf, ymax = Inf,
        #            alpha = .2,fill = "#008000") + 
        #             geom_rect(xmin = minP_oi_act - minP_oi_sd, xmax = minP_oi_act + minP_oi_sd, ymin = -Inf, ymax = Inf,
    ...
    #                                     trans = "log10",
    #                                    low = '#42047e',high = '#07f49e', na.value = 'gainsboro') +
            scale_y_continuous(breaks = c(60, 80, 100), limits = c(60, 100))+ theme_classic() + 
                labs(y = 'seq. id.')
        if(is.null(labels)){
            seq_bar <- seq_bar + 
                        theme(legend.position = 'bottom',
            ...
            seq_bar <- seq_bar + scale_x_discrete(labels = labels)
            if(pic_labels){
                seq_bar <- seq_bar + theme(legend.position = 'bottom',legend.box="horizontal", legend.margin=margin(),
                                  axis.text.x = element_markdown(color = "black", size = 6),
                                  axis.line.x = element_blank(),
                                  axis.ticks.x = element_blank(),
                                  axis.title.x=element_blank())
            }else{
                seq_bar <- seq_bar + theme(axis.text.x = element_text(angle = 45, hjust = 1))
            }
        }
    }else{
        seq_bar <- ggplot(species_act, aes(x=seqid, y=species)) + #, color=mean_MPRA_act)) + 
            geom_point(size = 2) + 
        #         geom_rect(xmin = cre_oi_wt_act-cre_oi_wt_sd, xmax = cre_oi_wt_act+cre_oi_wt_sd, ymin = -Inf, ymax = Inf,
        #            alpha = .2,fill = "#008000") + 
        #             geom_rect(xmin = minP_oi_act - minP_oi_sd, xmax = minP_oi_act + minP_oi_sd, ymin = -Inf, ymax = Inf,
    ...
    #                                     trans = "log10",
    #                                    low = '#42047e',high = '#07f49e', na.value = 'gainsboro') +
            scale_x_log10(breaks = c(60, 80, 100), limits = c(60, 100)) + theme_classic() + 
                labs(x = 'seq. id.')
        if(is.null(labels)){
            seq_bar <- seq_bar + 
                        theme(legend.position = 'bottom',
            ...
            seq_bar <- seq_bar + scale_y_discrete(labels = labels)
            if(pic_labels){
                seq_bar <- seq_bar + theme(legend.position = 'bottom',legend.box="horizontal", legend.margin=margin(),
                                  axis.text.y = element_markdown(color = "black", size = 6),
                                  axis.line.y = element_blank(),
                                  axis.ticks.y = element_blank(),
                                  axis.title.y=element_blank())
            }else{
...
make_aff_corr_df <- function(msa_tfbs_df, func_tfbs_df, query_name, ref_name){
    ...
    ref_tfbs_df <- left_join(ref_tfbs_df, func_tfbs_df %>% dplyr::select(TF_name, msa_start, msa_end, rank_TF),
                                by = c("TF_name",'msa_start','msa_end'))
...
#                 labs(x = 'Mouse binding affinity', y = 'First Ancestor\nbinding affinity') + 
                ...
                geom_text(data = comp_aff_corr, 
                          aes(label = paste0("rho = ", round(R, 2))),
                          x= -Inf, y = Inf,
                          hjust = -0.3,  # Align to the right
                        vjust = 1.5) +   # Align to the bottom  
...
plot_aff_trace <- function(msa_tfbs_df, func_tfbs_df, query_name, ref_name){
    ...
    aff_plot <- ggplot() + 
                  ...
                  geom_rect(data = func_tfbs_df, 
                        ...
                        colour = "#cc0000", fill = NA, linetype = "dashed", alpha = 0.6) + 
...
make_msa_tfbs_tile_df <- function(fasta.file, tile.list, ref_name, CRE_oi, tfbs_df, func_tfbs_df){
    ...
    msa_df_TFBS <- make_cre_tfbs_alignment_dataframe(msa_df, tfbs_df %>% select(species, ori_CRE_id ,start = start_pos, end = end_pos, TF_name, TFBS_orientation, norm_affinity), CRE_oi)
    msa_ref_df_TFBS <- msa_df_TFBS %>% filter(species == ref_name)
    msa_ref_df_TFBS_gr <- GRanges(msa_ref_df_TFBS %>% dplyr::select(chr = TF_name, start = msa_start, end = msa_end))
...
make_aff_corr_df2 <- function(msa_tfbs_df, func_tfbs_df, query_name, ref_name){
    query_tfbs_df <- msa_tfbs_df %>% filter(species == query_name) %>% 
            dplyr::select(TF_name, msa_start, msa_end, norm_affinity, TFBS_orientation)
    query_tfbs_gr <- GRanges(query_tfbs_df %>% dplyr::select(chr = TF_name, start = msa_start, end = msa_end))
