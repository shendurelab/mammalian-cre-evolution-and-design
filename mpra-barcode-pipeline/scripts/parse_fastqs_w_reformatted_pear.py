...
parser.add_argument('--in_UMI')
parser.add_argument('--R1_name')
parser.add_argument('--R2_name')
parser.add_argument('--UMI_name')
...
IN_UMI_file = args.in_UMI
R1_name = args.R1_name
R2_name = args.R2_name
UMI_name = args.UMI_name
...
with gzip.open(IN_R1_file, 'rt') as R1_fq, \
        gzip.open(IN_R2_file, 'rt') as R3_R1_rev_fq, \
        gzip.open(IN_UMI_file, 'rt') as UMI_fq, \
    ...
    if simple_out_bool:
        out_valid.write("\t".join([UMI_name, R1_name+'_pear']) + '\n')
        out_no_pear_BC.write("\t".join([UMI_name, R1_name+'_pear']) + '\n')
    else:
        out_valid.write("\t".join(['read_id', UMI_name, R1_name, R2_name, R1_name+'_pear']) + '\n')
        out_no_pear_BC.write("\t".join(['read_id', UMI_name, R1_name, R2_name, R1_name+'_pear']) + '\n')
    
    # handles to read files in
    h_R1_f, h_R3_R1_rc, h_R2 = [SeqIO.parse(e, 'fastq') for e in [R1_fq, R3_R1_rev_fq, UMI_fq]]
